...
function externalMintTierNft(
    address to,
    uint8 tierValue,
    string memory uri,
    uint256 paid
) external {
    // Cast tierValue -> Tier enum
    Tier finalTier = Tier(tierValue);

    // Call your existing public mint logic
    _mintTierNft(to, finalTier, uri, paid);
}
...
function _mintTierNft(
    ...
    string memory uri,
    uint256 paid
) public {  // <-- was "internal", now "public" or "external"
    _nextTokenId++;
