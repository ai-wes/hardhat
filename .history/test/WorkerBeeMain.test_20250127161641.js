...
describe("WorkerBeeNFTMain - Comprehensive Test Suite", function () {
  ...
  describe("Minting", () => {
    it("allows publicMint with insufficient payment", async () => {
      await expect(
        main.connect(user).publicMint("uri-for-worker", ethers.constants.HashZero, {
    ...
    it("finalizes VRF (publicMint) with the correct tier", async () => {
      ...
      await main.connect(user).publicMint("my-early-uri", ethers.constants.HashZero, {
        value: ethers.parseEther("0.01"),
      });
      let requestId = await vrf.lastRequestId();

      // Fulfill VRF request
      await vrf.fulfillRandomWords(requestId, main.address);
  ...
  describe("Fusion", () => {
    it("fuseBees requires user to own all tokens, etc.", async () => {
      ...
      await vrf.fulfillRandomWords(requestId1, main.address);
      await vrf.fulfillRandomWords(requestId1, main.address);

      await main.connect(user).mintDrone("drone2", { value: dronePayment });
      let requestId2 = await vrf.lastRequestId();
      await vrf.fulfillRandomWords(requestId2, main.address);
      await vrf.fulfillRandomWords(requestId2, main.address);
  ...
  describe("Decrees (Discount System)", () => {
    it("issueKingsDecree sets discount, usage limit, expiry, etc.", async () => {
      const decId = ethers.formatBytes32String("TEST");
      await expect(
  ...
  describe("Pause/Unpause", () => {
    ...
    it("publicMint fails when paused", async () => {
      await main.pause();
      await expect(
        main.connect(user).publicMint("xxx", ethers.constants.HashZero, {
          value: ethers.parseEther("0.01"),
        })
