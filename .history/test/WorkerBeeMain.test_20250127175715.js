...
describe("WorkerBeeNFTMain - Comprehensive Test Suite", function () {
  ...
  describe("Minting", () => {
    it("allows publicMint with insufficient payment", async () => {
      await expect(
        await main.connect(user).publicMint("uri-for-worker", ethers.ZeroHash, {
    ...
    it("finalizes VRF (publicMint) with the correct tier", async () => {
      ...
      await await main.connect(user).publicMint("my-early-uri", ethers.ZeroHash, {
        value: ethers.parseEther("0.01"),
      });
      ...
      expect(ownerOf1).to.equal(await main.getAddress());
  ...
  describe("Pause/Unpause", () => {
    ...
    it("publicMint fails when paused", async () => {
      await main.pause();
      await expect(
        await main.connect(user).publicMint("xxx", ethers.ZeroHash, {
          value: ethers.parseEther("0.01"),
        })
