...
describe("WorkerBeeNFTMain - Comprehensive Test Suite", function () {
  ...
  describe("Minting", () => {
    ...
    it("finalizes VRF (publicMint) with the correct tier (simulated)", async () => {
      ...
      // Tier enumeration: 0=Worker, 1=Drone, ...
      expect(beeInfo.tier).to.equal(1); // Drone
    });
  });
  ...
  describe("Revenue Sharing", () => {
    it("withdraw distributes funds according to revenueShares", async () => {
      ...
      // ethers v6: parse logs from rc.logs
      const parsedLogs = rc.logs
        .map((log) => {
          try {
            return main.interface.parseLog(log);
          } catch (err) {
            return null;
          }
        })
      ...
      // Filter for "RevenueWithdrawn" events
      const events = parsedLogs.filter((l) => l.name === "RevenueWithdrawn");
      expect(events.length).to.be.greaterThan(1);

      // Calculate expected distribution
      const expectedOwnerShare = ethers
        .parseEther("1")
        .mul(2000)
        .div(10000); // 20%
      const expectedKingBeeShare = ethers
        .parseEther("1")
        .mul(4500)
        .div(10000); // 45%
      const remain = ethers.parseEther("1").sub(
        expectedOwnerShare.add(expectedKingBeeShare)
      );

      // Check that deployer got at least the 20% share minus gas
      const newBalDeployer = await ethers.provider.getBalance(deployer.address);
      // We only check that it went up by about 0.2 ETH because we pay gas
      const difference = newBalDeployer - oldBalDeployer;
      expect(difference).to.be.closeTo(expectedOwnerShare, ethers.parseEther("0.001"));
    });
  });
