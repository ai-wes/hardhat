...
describe("WorkerBeeNFTMain - Comprehensive Test Suite", function () {
  ...
  describe("Revenue Sharing", () => {
    it("withdraw distributes funds according to revenueShares", async () => {
      ...
      // FIX #2: do BigInt math
      const oneEther = ethers.parseEther("1"); // bigint
      const expectedOwnerShare = (oneEther * 2000n) / 10000n; // 20%
      const expectedKingBeeShare = (oneEther * 4500n) / 10000n; // 45%
      const remain = oneEther - (expectedOwnerShare + expectedKingBeeShare);
  
      // Check difference with a tolerance
      const newBalDeployer = await ethers.provider.getBalance(deployer.address);
      const difference = newBalDeployer - oldBalDeployer;
  
      // e.g. within ~0.001 ETH => 10^15 wei
      expect(difference).to.be.closeTo(expectedOwnerShare, 10n ** 15n);
    });
  });
