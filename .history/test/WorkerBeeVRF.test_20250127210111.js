...
await workerBeeVRF.waitForDeployment();
  ...
  describe("VRF-Based Minting", function () {
    ...
    it("fulfillRandomWords finalizes a standard publicMint with Worker or Drone tier", async () => {
        ...
        const tx = await workerBeeVRF.connect(user).publicMint("myUri", ethers.constants.HashZero, {
        value: ethers.parseEther("0.01"),
        });
        const receipt = await tx.wait();
        // Extract the requestId from the event
        const requestId = receipt.events.find(event => event.event === 'MintRequestCreated').args.requestId;
        // Define a mock randomness value
        const mockRandomness = ethers.BigNumber.from("123456789"); // Example value
        // Fulfill VRF request with a valid array
        await vrf.fulfillRandomWords(requestId, [mockRandomness]);
        // Check totalMintedOverall
